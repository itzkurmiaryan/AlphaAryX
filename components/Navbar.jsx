...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 border-b shadow-xl backdrop-blur-2xl bg-white/90 border-white/20">
      <div className="flex flex-wrap items-center justify-between gap-4 px-4 py-4 mx-auto max-w-7xl">
        ...
        <div className="items-center hidden gap-3 md:flex">
          <button
            ...
            className="relative inline-flex items-center px-3 py-2 rounded-full bg-slate-100 text-slate-800 hover:bg-slate-200"
          >
          ...
          {!user ? (
            ...
            <>
              ...
              <Link href={dashboardLink}>
                <button className="px-4 py-2 text-sm text-white bg-green-500 rounded-full hover:bg-green-600">
                  Dashboard
                </button>
              </Link>
      ...
      {/* Mobile Menu */}
      {open && (
        <div className="px-4 pb-6 space-y-4 border-t md:hidden border-slate-200 bg-white/95">
          <div className="flex flex-col gap-3">
            {navLinks.map((link, i) => (
              <Link
                ...
                className="block px-4 py-3 text-base font-medium rounded-2xl text-slate-700 hover:bg-slate-100"
              >
                {link.name}
              </Link>
            ))}
          ...
          {!user ? (
            <div className="flex flex-col gap-3">
              <Link
                href="/login"
                onClick={() => setOpen(false)}
                className="block px-4 py-3 text-center border rounded-2xl border-slate-200"
              >
                Login
              </Link>
              ...
              <Link
                href="/register"
                onClick={() => setOpen(false)}
                className="block px-4 py-3 text-center text-white bg-indigo-600 rounded-2xl"
              >
                Register
              </Link>
            </div>
            ...
            <div className="flex flex-col gap-3">
              <span className="px-4 py-3 rounded-2xl bg-slate-100">
                Hi, {user.name}
              </span>
              ...
              <Link
                href={dashboardLink}
                onClick={() => setOpen(false)}
                className="block px-4 py-3 text-center text-white bg-green-500 rounded-2xl"
              >
                Dashboard
              </Link>
              ...
              <button
                onClick={logout}
                className="w-full px-4 py-3 text-sm font-semibold text-center text-white bg-red-500 rounded-2xl"
              >
                Logout
              </button>
            </div>
