...
export const initSocket = (httpServer) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("markAsRead", async (data) => {
      try {
        const { messageId, messageIds, userId } = data;
        if (messageIds && Array.isArray(messageIds) && messageIds.length > 0) {
          await Message.updateMany({ _id: { $in: messageIds } }, { isRead: true });
          messageIds.forEach((id) => {
            if (currentRoom) {
              io.to(currentRoom).emit("messageRead", { messageId: id, readerId: userId });
            }
          });
        } else if (messageId) {
          await Message.findByIdAndUpdate(messageId, { isRead: true });
          if (currentRoom) {
            io.to(currentRoom).emit("messageRead", { messageId, readerId: userId });
          }
        }
      } catch (error) {
