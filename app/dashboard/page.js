...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen p-6 text-white bg-slate-950">
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="p-8 border rounded-3xl border-white/10 bg-white/5">
          ...
          <div className="grid gap-4 mt-8 sm:grid-cols-3">
            <div className="p-6 border rounded-3xl border-white/10 bg-slate-900/80">
        ...
        <div className="p-6 mt-8 border rounded-3xl border-cyan-500/20 bg-cyan-500/5">
          <h3 className="text-xl font-bold text-cyan-300">
            Order Policy
          </h3>

          <ul className="mt-4 space-y-2 text-slate-300">
            <li>✅ 50% advance payment before project starts</li>
            <li>✅ Remaining 50% after project completion</li>
            <li>✅ Completed orders cannot be cancelled</li>
            <li>✅ Paid orders cannot be cancelled</li>
            <li>✅ Dedicated customer support available</li>
          </ul>
        </div>
        ...
        <div className="p-8 border rounded-3xl border-white/10 bg-white/5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link href="/services" className="inline-flex items-center justify-center px-5 py-3 text-sm font-semibold rounded-full bg-cyan-500 text-slate-950 hover:bg-cyan-400">
              Browse services
            </Link>
          </div>
          ...
          {loading ? (
            ...
            <div className="p-8 mt-8 text-center border border-dashed rounded-3xl border-white/10 bg-slate-900/70 text-slate-300">
            ...
            <div className="mt-8 space-y-4">
              {orders.map((order) => (
                <div key={order._id || order.id} className="p-6 border rounded-3xl border-white/10 bg-slate-900/80">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
  ...
  <p
    className={`mt-2 inline-flex rounded-full px-3 py-1 text-xs uppercase tracking-[0.15em]
      ${
  ...
  >
    {order.paymentStatus || "PENDING"}
                  ...
                  <div className="flex gap-3 mt-4">
                    ...
                    {order.status === "CANCELLED" && (
                      <button
                        ...
                        className="px-4 py-2 text-sm font-semibold bg-white rounded-full text-slate-900"
                      >
                        Delete
                      </button>
                    )}

                    <Link href={`/orders/${order._id || order.id}`} className="px-4 py-2 text-sm font-semibold text-white rounded-full bg-cyan-600 hover:bg-cyan-500">
                      View Details
                    </Link>
                  </div>
                </div>
        ...
        <div className="p-8 border rounded-3xl border-white/10 bg-white/5">
          <h2 className="mb-4 text-2xl font-semibold">Chat with Admin</h2>
          <p className="mb-6 text-slate-400">Get direct support from our team</p>
          <ChatComponent
